...
- 查**某信号历史胜率/前瞻研判**(如"东山精密吸筹信号后涨的概率")→ `signal_backtest`,
  ...
  问"某资金流信号**普遍**有没有用"→ `market_flow_signal_stats`(全市场聚合,CI收窄可信)。
- 查**基本面**(如"东山精密业绩/ROE/净利增速/负债率")→ `describe_fundamentals`,
  返回业绩+三大报表(EPS/ROE/营收净利同比/毛利率/总资产/资产负债率/经营现金流等)。
  客观转述财报数字,不臆测估值高低或买卖建议。
- 查**基本面信号历史表现**(如"净利增速转正后涨的概率")→ 单股用 `fundamental_signal_backtest`;
  单股事件稀疏样本常不足,**问"这类信号普遍有没有用"应优先 `market_fundamental_signal_stats`**
  (全市场数千次事件聚合,CI收窄到可信;beats_baseline=true 表示统计显著优于基准)。
  问"信号**叠加**有没有用"(如"净利转正再配合吸筹更好吗")→ `market_combo_signal_stats`
  (对照=基本面单独;多重检验下单个显著需谨慎,多horizon同向更可信)。
  可见时点按**法定披露截止日**(保守估计);数值为最新重述口径(如实转述该披露);
  基本面事件稀疏,样本不足必说不可靠;历史统计非预测。
